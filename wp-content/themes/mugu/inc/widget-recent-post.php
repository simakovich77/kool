...
// register Mugu_Recent_Post widget
function mugu_register_recent_post_widget() {
    register_widget( 'Mugu_Recent_Post' );
}
add_action( 'widgets_init', 'mugu_register_recent_post_widget' );
 
 /**
 * Adds Mugu_Recent_Post widget.
 */
class Mugu_Recent_Post extends WP_Widget {
    ...
    function __construct() {
        parent::__construct(
            'mugu_recent_post', // Base ID
            __( 'RARA: Recent Post', 'mugu' ), // Name
            array( 'description' => __( 'A Recent Post Widget', 'mugu' ), ) // Args
        );
    }
    ...
    public function widget( $args, $instance ) {
        ...
        $title      = ! empty( $instance['title'] ) ?  $instance['title'] : __( 'Recent Posts', 'mugu' );
                            ...
                            <?php if( $show_date ) { ?>
                                <div class="entry-meta">
                                    <span class="posted-on"><a href="<?php the_permalink(); ?>">
                                        <time datetime="<?php printf( __( '%1$s', 'mugu' ), get_the_date('Y-m-d') ); ?>"><?php printf( __( '%1$s', 'mugu' ), get_the_date('F j, Y') ); ?></time></a>
                                    </span>
                                </div>
                            <?php } ?>
                        </div>                        
    ...
    public function update( $new_instance, $old_instance ) {
        ...
        $instance['title']          = ! empty( $new_instance['title'] ) ? sanitize_text_field( $new_instance['title'] ) : __( 'Recent Posts', 'mugu' );
