...
// register Mugu_Featured_Post widget
function mugu_register_featured_post_widget() {
    register_widget( 'Mugu_Featured_Post' );
}
add_action( 'widgets_init', 'mugu_register_featured_post_widget' );
 
 /**
 * Adds Mugu_Featured_Post widget.
 */
class Mugu_Featured_Post extends WP_Widget {
	...
	function __construct() {
		parent::__construct(
			'mugu_featured_post', // Base ID
			__( 'RARA: Featured Post', 'mugu' ), // Name
			array( 'description' => __( 'A Featured Post Widget', 'mugu' ), ) // Args
		);
	}
	...
	public function widget( $args, $instance ) {
        $read_more      = !empty( $instance['readmore'] ) ? $instance['readmore'] : __( 'Read More', 'mugu' );		     
            ...
            ?>
                ...
                <div class="img-holder">
                    <a href="<?php the_permalink(); ?>">
                        <?php the_post_thumbnail( 'mugu-featured-post' ); ?>
                    </a>
                </div>    				
                <?php } ?>
	...
	public function form( $instance ) {
        ...
        ?>
		<p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'post_list' ) ); ?>"><?php esc_html_e( 'Posts', 'mugu' ); ?></label>
        ...
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'readmore' ) ); ?>"><?php esc_html_e( 'Read More Text', 'mugu' ); ?></label> 
	...
	public function update( $new_instance, $old_instance ) {
        ...
        $instance['readmore']       = ! empty( $new_instance['readmore'] ) ? sanitize_text_field( $new_instance['readmore'] ) : __( 'Read More', 'mugu' );
